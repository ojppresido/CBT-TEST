...
def remove_mathjax_inline(text):
    ...
    # Handle basic MathJax commands by removing the backslashes
...
def fix_trigonometry_diagram(svg_content):
    ...
    # Opposite: from (100,150) to (100,80) (vertical)
    
    # Update the triangle points to form a proper right triangle
    fixed_svg = svg_content.replace(
    ...
    # Update the right angle marker to be at the right position (bottom-left corner)
    fixed_svg = fixed_svg.replace(
    ...
    # Original: 3 (was labeled on horizontal), 4 (was labeled on horizontal), 5 (was on diagonal)
    # New: 3 (vertical side), 4 (horizontal side), 5 (hypotenuse)
    fixed_svg = fixed_svg.replace(
        '<text x=\"90\" y=\"155\" font-size=\"14\">3</text>',  # Originally on horizontal, now vertical
